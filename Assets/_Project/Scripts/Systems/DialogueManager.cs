...
public class DialogueManager : MonoBehaviour
{
    ...
    public void StartDialogue(TMP_Text textUI, Transform Target, string text, float lookDuration, bool lookAtTarget)
    {
        if (typeCoroutine != null) StopCoroutine(typeCoroutine);
        if (lookCoroutine != null) StopCoroutine(lookCoroutine);

        typeCoroutine = StartCoroutine(TypingText(textUI, text));
        if (lookAtTarget && Target != null && panTiltComponent != null)
        {
            lookCoroutine = StartCoroutine(LookAtTarget(Target, lookDuration));
        }
    ...
    IEnumerator TypingText(TMP_Text textComponent, string text)
    {
    ...
    IEnumerator LookAtTarget(Transform target, float duration)
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            if (dirToTarget != Vector3.zero)
            {
                ...
                float timeRemaning = duration - elapsedTime;
                float frameStep = Time.deltaTime / Mathf.Max(0.5f, timeRemaning);

                panTiltComponent.PanAxis.Value += deltaPan * frameStep;
                panTiltComponent.TiltAxis.Value += deltaTilt * frameStep;
            }
